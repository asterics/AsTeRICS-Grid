...
This chapter explains the most important terms used in relation to the Asterics AAC.
...
## AsTeRICS Framework
Java-based software suite that runs on a computer and makes it possible to trigger additional actions using the Asterics AAC, e.g. controlling a TV (environmental control). See [AsTeRICS Framework](https://www.asterics.eu/get-started/) on the web.

## AsTeRICS model
An AsTeRICS model is a configuration for the AsTeRICS Framework which realizes a certain use-case, e.g. environmental control, camera based head-tracking or eye-tracking. A model that runs in the AsTeRICS Framework can interact with the Asterics AAC.

## Grid set
Term for a set of pages in the Asterics AAC, consisting of several grids. The number of grids a grid set contains is flexible.

## Grid
Term for a single page in the Asterics AAC, consisting of several grid elements. The number of elements a grid contains is flexible.
...
## Input options
In the context of Asterics AAC "input options" refer to the mean of selecting a grid element. This can be e.g. a normal mouse click, hovering or scanning. See [Input Options](09_input-options.md).
...
## User
In Asterics AAC a "user" refers to a username and a set of grids, which in sum realize a solution for an actual user. There are two types of users in Asterics AAC:
