...
This chapter is about the general structure of the Asterics AAC project:
...
* **scripts/**: folder containing shell scripts, e.g. for releasing a new Asterics AAC release
* **src/**: folder containing Javascript sources 
* **superlogin/**: folder containing configuration of [couch-auth](https://github.com/perfood/couch-auth) (former superlogin), a library that handles users and databases for [CouchDB](http://couchdb.apache.org/), the database backend used by Asterics AAC.
...
* **package.json**: [npm](https://www.npmjs.com/) configuration file specifying dependencies and scripts for the Asterics AAC project
* **README.md**: markdown readme file for the GitHub project page 
* **serviceWorker.js**: [Service Worker](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) that manages offline capabilities of Asterics AAC using [Workbox](https://github.com/GoogleChrome/workbox)
* **unsupported.html**: static HTML file that is shown if Asterics AAC is opened with an unsupported browser
...
Besides the sources in the `src/` folder the `app/` folder contains these elements which in total realize the working Asterics AAC application:
...
* **simple/**: folder containing a simple version of Asterics AAC which only shows simple buttons which can trigger ARE actions defined in a gridset (`.grd`) file (archived).
