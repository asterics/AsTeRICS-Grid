    ...
    <div class="box" id="gridView" v-cloak>
        ...
        <div class="srow content" v-show="gridData.gridElements && (gridData.gridElements.length > 0 || (globalGridData && globalGridData.gridElements.length > 0))">
            ...
            <div id="grid-container" class="grid-container" :style="`background-color: ${backgroundColor}`">
                <div v-for="element in gridData.gridElements" :key="element.id" class="grid-item-content" :class="{ crossed: isCrossed(element.id) && enableCrossing }">
...
let vueConfig = {
    ...
    data() {
        return {
            ...
            enableCrossing: false // New property to enable/disable crossing
        }
    },
    components: {
        UnlockModal,
...
/* Add a class for crossed out elements */
...
.grid-item-content.crossed::before {
    ...
    z-index: 10;
    pointer-events: none; /* Make sure it doesn't interfere with clicking the button */
}
</style>
