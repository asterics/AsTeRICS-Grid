        ...
        <div class="modal-mask">
            <div class="modal-wrapper">
                <div class="modal-container" @keyup.27="$emit('close')" @keyup.ctrl.enter="save()">
                    <a class="inline close-button" href="javascript:void(0);" @click="$emit('close')"><i
                    ...
                    <div class="modal-body">
                        <div class="srow">
                            ...
                            <select v-if="editElement" class="four columns" id="selectMode" type="checkbox"
                                v-model="editElement.mode">
                        ...
                        <div class="srow">
                            ...
                            <input v-if="editElement" class="four columns" id="imageHeight" type="number" min="50"
                                max="100" :disabled="!editElement.showLabels || !autoOrSeparatedMode"
                                v-model.number="editElement.imageHeightPercentage" />
                        </div>
                        <div class="srow">
                            <label for="singleTextElemFactor" class="five columns">{{
                                    $t('factorForFontSizeOfOnlytextElements')
                            }}</label>
                            <input v-if="editElement" class="four columns" id="singleTextElemFactor" type="number"
                                min="0" max="5" step="0.1" :disabled="!autoOrSeparatedMode"
                                v-model.number="editElement.textElemSizeFactor" />
                        </div>
                        <div class="srow">
                            <input v-if="editElement" id="showLabel" type="checkbox" v-model="editElement.showLabels"
                                :disabled="!autoOrSeparatedMode" />
                            <label for="showLabel">{{ $t('showLabelsOfCollectedImages') }}</label>
                        </div>
                        <div class="srow">
                            <input v-if="editElement" id="useSingleLine" type="checkbox"
                                v-model="editElement.singleLine" :disabled="!autoOrSeparatedMode" />
                            <label for="useSingleLine">{{ $t('useHorizontalScrollbarIfElementsDontFit') }}</label>
                        </div>
                        <div class="srow">
                            <input v-if="editElement" id="convertToLowercase" type="checkbox"
                                v-model="editElement.convertToLowercase" />
                            <label for="convertToLowercase">{{ $t('convertUppercaseKeyboardLettersToLowercase')
                            }}</label>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer">
                        <div class="button-container srow">
                            <button @click="$emit('close')" :title="$t('keyboardEsc')"
                                class="four columns offset-by-four">
