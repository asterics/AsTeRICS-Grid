<template>
    <div class="container-fluid px-0">
        <div class="row">
            <div class="col-12">
                <input v-if="gridElement" id="preventClick" type="checkbox" v-model="gridElement.additionalProps[GridElement.PROP_YT_PREVENT_CLICK]"/>
                <label for="preventClick">{{ $t('todo') }}</label>
            </div>
        </div>
    </div>
</template>
