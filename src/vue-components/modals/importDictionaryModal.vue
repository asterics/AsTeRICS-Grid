<template>
    <base-modal icon="fas fa-file-import" :title="$t('importDictionary')" :help="false" @open="init" v-on="$listeners">
                    <template #default>
                    ...
                    <template #ok>
                            <button class="btn-primary" @click="save" :title="$t('keyboardCtrlEnter')" :aria-label="$t('importDictionary')" :disabled="isImportDisabled">
                                <i class="fas fa-check" aria-hidden="true"></i><span>{{ $t('importDictionary') }}</span> <i class="fas fa-spinner fa-spin" v-show="loading" aria-hidden="true"></i>
                            </button>
                    </template>
    </base-modal>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            isImportDisabled() {
                return this.importType === this.c.SELECT_ONLINE && !this.selectedOption || this.importType === this.c.SELECT_FILE && !this.selectedFile;
            }
        },
        methods: {
            save() {
