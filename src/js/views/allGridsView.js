...
/**
 * class AllGridsView, controller for view "allGridsView"
 * @constructor
 */
function AllGridsView() {
    let AllGridsViewInstance = this;
    let vueApp = null;
     ...
     * @return {AllGridsView}
     */
    AllGridsViewInstance.init = function () {
        $(document).on(constants.EVENT_DB_PULL_UPDATED, reloadFn);
        dataService.getGrids(true).then(grids => {
            log.debug(grids);
            initVue(grids);
        });
        return AllGridsViewInstance;
    };

    /**
     * destroys the view
     */
    AllGridsViewInstance.destroy = function () {
